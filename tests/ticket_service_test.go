...
func TestTicketServiceSearchExistedRecordOrganizationName(t *testing.T) {
	testcases := []SearchTestCase{
		...
		{"Search by existed _id organization name", SearchArgs{"_id", "27c447d9-cfda-4415-9a72-d5aa12942cf1"}, "Bitrex", false},
		{"Search by existed url organization name", SearchArgs{"url", "http://initech.tokoin.io.com/api/v2/tickets/27c447d9-cfda-4415-9a72-d5aa12942cf1.json"}, "Bitrex", false},
		{"Search by existed external_id organization name", SearchArgs{"external_id", "b17a9d1b-bc80-4262-a387-bb4f4209d7e2"}, "Bitrex", false},
		{"Search by existed subject organization name", SearchArgs{"subject", "A Problem in Ethiopia"}, "Bitrex", false},
		{"Search by existed description organization name", SearchArgs{"description", "Ex sit ea sit exercitation tempor pariatur et do deserunt irure eiusmod. Exercitation anim consectetur amet anim id."}, "Bitrex", false},
		{"Search by existed status organization name", SearchArgs{"status", "hold"}, "Bitrex", false},
		{"Search by existed submitter_id organization name", SearchArgs{"submitter_id", "67"}, "Bitrex", false},
		{"Search by existed assignee_id organization name", SearchArgs{"assignee_id", "55"}, "Bitrex", false},
	}
